...
export default class AnimateCharacter {
  ...
  addTransformControl = (bone) => {
    let control = new TransformControls(
      this._editor.camera,
      this._editor.renderer.domElement
    );
    control.attach(bone);
  ...
  setupSkeletonControllers = () => {
    ...
    let rootBone = this._fbxModel.children.find(
      (child) => child.type === "Bone"
    );
    if (rootBone) {
      ...
      if (index >= 0) {
        ...
        rootBone.traverse((child) => {
          ...
          if (child.type === "Bone") {
            ...
            let bonesList = ["LeftArm", "RightArm", "LeftLeg", "RightLeg"];
            bonesList = bonesList.map((v) => `${modelName}${v}`);
            if (bonesList.indexOf(parentBone) >= 0) {
              //   this.addTransformControl(child);
            }
            bonesList = ["LeftHand", "RightHand", "Neck"];
            bonesList = bonesList.map((v) => `${modelName}${v}`);
      ...
      if (animations.length > 0) {
        ...
        if (targetAnimation) {
          this._currentAnimation =
            this._animationMixer.clipAction(targetAnimation);
