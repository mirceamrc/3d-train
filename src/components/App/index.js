...
class App extends React.Component {
  ...
  computeHeadRotation(face) {
    const { origin, rotationMatrix } =
      GeometryUtil.computeHeadPoseEstimation(face);
    const { pitch, yaw, roll } =
      math.rotationMatrixToEulerAngles(rotationMatrix);
      ...
      try {
        ...
        if (cam.isRunning) {
          if (faces && faces.length > 0) {
            ...
            ctx.restore();
            let faceLoc = faces[0];
            const { origin, rotationMatrix, pitch, yaw, roll } =
